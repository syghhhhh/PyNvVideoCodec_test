...
class GpuNV12FramePool:
    ...
    def __init__(self, width, height):
        ...
        # 使用 zeros 而不是 empty，确保初始化为0
        self._buf = cp.zeros(y_size + uv_size, dtype=cp.uint8)
...
def merge_video(item, id_merge, ret_dict, config_dict, log_file):
    ...
    frame_pool = None
    h264_file = None
    human_frame_gen = None
    node_bottom_gen_list = []
    node_top_gen_list = []
    ...
    try:
        ...
        for i in range(frame_num):
            ...
            torch.cuda.synchronize()            # 或 cp.cuda.runtime.deviceSynchronize()
        ...
        # ==================== 资源清理 ====================
        log_content_write(log_file, 'start cleanup resources')
        ...
        # 2. 关闭生成器
        if human_frame_gen is not None:
            try:
                human_frame_gen.close()
            except:
                pass
        
        for gen in node_bottom_gen_list:
            try:
                gen.close()
            except:
                pass
                
        for gen in node_top_gen_list:
            try:
                gen.close()
            except:
                pass
